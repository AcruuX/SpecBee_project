...
"use strict";var $=require("gulp-load-plugins")(),del=require("del"),extend=require("extend"),fs=require("fs"),gulp=require("gulp"),importOnce=require("node-sass-import-once"),options={};if(options.gulpWatchOptions={},options.rootPath={project:__dirname+"/",theme:__dirname+"/"},options.theme={root:options.rootPath.theme,scss:options.rootPath.theme+"scss/",css:options.rootPath.theme+"css/"},options.scss={importer:importOnce,outputStyle:"compressed",lintIgnore:["scss/_settings.scss","scss/base/_drupal.scss"],includePaths:[options.rootPath.project+"node_modules/bootsrap/scss",options.rootPath.project+"node_modules/motion-ui/src"]},options.autoprefixer={browsers:["last 2 versions","ie >= 9"]},fs.existsSync(options.rootPath.project+"/config.js")){var config={};config=require("./config"),extend(!0,options,config)}var scssFiles=[options.theme.scss+"**/*.scss","!"+options.theme.scss+"**/_*.scss"];gulp.task("default",["build"]),gulp.task("build",["sass","lint"]),gulp.task("watch",["watch:css"]),gulp.task("watch:css",["sass","lint:sass"],function(){return gulp.watch(options.theme.scss+"**/*.scss",options.gulpWatchOptions,["sass","lint:sass"])}),gulp.task("lint",["lint:sass"]),gulp.task("sass",["clean:css"],function(){return gulp.src(scssFiles).pipe($.sourcemaps.init()).pipe($.sass(extend(!0,{noCache:!0,outputStyle:options.scss.outputStyle,sourceMap:!0},options.scss)).on("error",$.sass.logError)).pipe($.autoprefixer(options.autoprefixer)).pipe($.rename({dirname:""})).pipe($.size({showFiles:!0})).pipe($.sourcemaps.write("./")).pipe(gulp.dest(options.theme.css))}),gulp.task("clean:css",function(){return del([options.theme.css+"**/*.css",options.theme.css+"**/*.map"],{force:!0})}),gulp.task("lint:sass",function(){return gulp.src(options.theme.scss+"**/*.scss").pipe($.sassLint({files:{include:$.cached("scsslint"),ignore:options.scss.lintIgnore}})).pipe($.sassLint.format())}),function(s,t,e){t.behaviors.myModuleBehavior={attach:function(t,e){s("#tabs-nav li:first-child").addClass("active"),s(".tab-content").hide(),s(".tab-content:first").show(),s("#tabs-nav li").click(function(){s("#tabs-nav li").removeClass("active"),s(this).addClass("active"),s(".tab-content").hide();var t=s(this).find("a").attr("href");return s(t).fadeIn(),!1}),s(".tab-slider-wrapper").slick({dots:!0,infinite:!1,speed:300,slidesToShow:3,responsive:[{breakpoint:1024,settings:{slidesToShow:3,infinite:!0}},{breakpoint:768,settings:{slidesToShow:2}},{breakpoint:480,settings:{slidesToShow:1,dots:!1}}]})}}}(jQuery,Drupal,once),$(".responsive").slick({dots:!0,infinite:!1,speed:300,slidesToShow:4,slidesToScroll:4,responsive:[{breakpoint:1024,settings:{slidesToShow:3,slidesToScroll:3,infinite:!0,dots:!0}},{breakpoint:600,settings:{slidesToShow:2,slidesToScroll:2}},{breakpoint:480,settings:{slidesToShow:1,slidesToScroll:1}}]});;
/*
     _ _      _       _
 ___| (_) ___| | __  (_)___
/ __| | |/ __| |/ /  | / __|
